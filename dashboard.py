...
def get_db_connection(db_path: str = "data/despesas.db"):
...
def tab_visao_geral(df: pd.DataFrame, stats_fase: pd.DataFrame):
    ...
    # KPIs responsivos (Streamlit lida com responsividade automaticamente)
...
def main():
    ...
    if not os.path.exists("data/despesas.db"):
        st.error("Banco de dados não encontrado. Execute o scraper primeiro:")
        st.code("python3 scraper.py", language="python")
        return
    ...
    if periodo is None or periodo['data_min'] is None:
        st.warning("Nenhum dado disponível. Execute o scraper para coletar dados.")
        return
