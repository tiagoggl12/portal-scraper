...
def ensure_database():
    ...
    # Download from GitHub if not exists or invalid
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            st.error(f"Banco de dados não encontrado (HTTP {response.status_code})")
            return False
...
def carregar_dados(
    filtro_fase: list = None,
    filtro_orgao: list = None,
...
def get_estatisticas_gerais():
    ...
    try:
        ...
        return (
            stats_fase,
            periodo.iloc[0] if not periodo.empty else None,
            orgaos["orgao"].tolist(),
        )
    except:
        return pd.DataFrame(), None, []
...
def top_favorecidos_table(df: pd.DataFrame, top_n: int = 20):
    ...
    df_fav = (
        df[df["favorecido"].notna()]
        .groupby("favorecido")["valor_pago"]
        .sum()
        .reset_index()
    )
    ...
    for idx, row in df_fav.iterrows():
        ...
        with col1:
            st.write(
                f"**{row['favorecido'][:40]}**"
                if len(row["favorecido"]) > 40
                else f"**{row['favorecido']}**"
            )
        with col2:
...
def render_sidebar(periodo, lista_orgaos, stats_fase):
    ...
    # Filtro de fase
    fase_disponivel = (
        stats_fase["fase"].tolist() if not stats_fase.empty else ["pagamento"]
    )
    ...
    filtro_orgao = st.sidebar.multiselect(
        "Selecione órgãos", options=orgaos_filtrados, default=[], key="filtro_orgao"
    )
...
def tab_visao_geral(df: pd.DataFrame, stats_fase: pd.DataFrame):
    ...
    with col_left:
        ...
        df_dia = (
            df[df["data_pagamento"].notna()]
            .groupby("data_pagamento")["valor_pago"]
            .sum()
            .reset_index()
        )
        ...
        df_dia["data_formatada"] = pd.to_datetime(df_dia["data_pagamento"]).dt.strftime(
            "%d/%m/%Y"
        )
...
def tab_analise_temporal(df: pd.DataFrame):
    ...
    df_dia = (
        df_temp.groupby("data_pagamento")
        .agg(
            {
                "valor_empenhado": "sum",
                "valor_liquidado": "sum",
    ...
    df_dia["media_movel_7d"] = (
        df_dia["valor_pago"].rolling(window=7, min_periods=1).mean()
    )
    ...
    with col2:
        ...
        st.metric(
            "**Pico (Data)**",
            pico["data_pagamento"].strftime("%d/%m"),
    ...
    with col3:
        st.metric(
            "**Total Período**",
            formatar_moeda(df_dia["valor_pago"].sum()),
            f"{len(df_dia)} dias",
        )
...
def criar_grafo_correlacoes(
    df: pd.DataFrame, min_correlacao: float = 0.3, top_n_orgaos: int = 15
):
    """
    ...
    top_orgaos = (
        df.groupby("orgao")["valor_pago"].sum().nlargest(top_n_orgaos).index.tolist()
    )
    ...
    for i, orgao in enumerate(top_orgaos):
        ...
        G.add_node(
            f"org_{i}", label=nome_curto, type="orgao", nome_orgao=orgao, size=20
        )
        ...
        for j, var2 in enumerate(vars_numericas):
            if i < j:
                ...
                if abs(corr) >= min_correlacao:
                    G.add_edge(
                        var1, var2, weight=abs(corr), correlation=corr, tipo="variavel"
                    )
    ...
    for i, orgao1 in enumerate(top_orgaos):
        for j, orgao2 in enumerate(top_orgaos):
            if i < j:
                ...
                df_org1 = df[df["orgao"] == orgao1][
                    ["data_pagamento", "valor_pago"]
                ].dropna()
                df_org2 = df[df["orgao"] == orgao2][
                    ["data_pagamento", "valor_pago"]
                ].dropna()
    ...
    color_map = {"variavel": "#00a195", "orgao": "#ff6b6b"}
...
def criar_grafo_outras_correlacoes(df: pd.DataFrame, tipo: str = "variaveis"):
    ...
    if tipo == "variaveis":
        ...
        top_orgaos = (
            df.groupby("orgao")["valor_empenhado"].sum().nlargest(8).index.tolist()
        )
        ...
        # Cores para órgãos
        colors_orgaos = [
            f"rgba(0, {100 + i * 15}, 149, 0.5)" for i in range(len(top_orgaos))
        ]
        ...
        fig.update_layout(
            title="Fluxo Financeiro: Empenhado → Pago (Top 8 Órgãos)", height=600
        )
...
def tab_correlacoes(df: pd.DataFrame):
    ...
    with subtab1:
        ...
        if tipo_grafo == "Grafo de Rede":
            fig = criar_grafo_correlacoes(
                df, min_correlacao=min_corr, top_n_orgaos=top_n
            )
            st.plotly_chart(fig, use_container_width=True)
    ...
    with subtab3:
        ...
        df_orgao = (
            df.groupby("orgao")[["valor_empenhado", "valor_pago"]].sum().reset_index()
        )
        ...
        if not df_orgao.empty:
            ...
            max_val = max(
                df_orgao["valor_empenhado"].max(), df_orgao["valor_pago"].max()
            )
    ...
    with subtab4:
        ...
        for orgao in df["orgao"].unique():
            ...
            if len(df_org) > 1:
                corr = (
                    df_org[vars_numericas].corr().loc["valor_pago", "valor_empenhado"]
                )
                correlations.append(
                    {
                        "orgao": orgao[:30],
...
def tab_dados_detalhados(df: pd.DataFrame):
    ...
    if busca:
        filtro = (
            df["favorecido"].str.contains(busca, case=False, na=False)
    ...
    df_display["valor_empenhado"] = df_display["valor_empenhado"].apply(
        lambda x: f"R$ {x:,.2f}" if pd.notna(x) else "-"
    )
    df_display["valor_liquidado"] = df_display["valor_liquidado"].apply(
        lambda x: f"R$ {x:,.2f}" if pd.notna(x) else "-"
    )
...
def main():
    # Header
    st.markdown(
        '<h1 class="main-header">:cityscape: Portal da Transparência de Fortaleza</h1>',
        unsafe_allow_html=True,
    )
    ...
    if not ensure_database():
        st.warning(
            "Banco de dados em carregamento. Tente novamente em alguns instantes."
        )
        return
    ...
    filtro_fase, filtro_orgao, data_inicio, data_fim = render_sidebar(
        periodo, lista_orgaos, stats_fase
    )
    ...
    with cols_info[0]:
        st.caption(
            f"Período: **{formatar_data_brasil(periodo['data_min'])}** a **{formatar_data_brasil(periodo['data_max'])}**"
        )
    with cols_info[1]:
